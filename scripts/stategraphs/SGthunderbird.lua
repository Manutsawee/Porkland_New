...
local events = {
    ...
    EventHandler("cancel_charge", function(inst)
...
local states = {
    State{
        ...
        onenter = function(inst)
            if inst.thunderbird_fx and math.random() < 0.1 then
                inst.thunderbird_fx.AnimState:PlayAnimation("idle")
            end
    ...
    State{
        ...
        onenter = function(inst)
            ...
            if homeseeker ~= nil then
                ...
                home.components.pickable:Regen()
                inst.sg:GoToState("idle")
            end
        end,
    },
    ...
    State{
        ...
        onenter = function(inst)
            if inst.thunderbird_fx then
                inst.thunderbird_fx.AnimState:PlayAnimation("charge_pre")
                inst.thunderbird_fx.AnimState:PushAnimation("charge_loop", true)
            end
    ...
    State{
        ...
        onenter = function(inst)
            if inst.thunderbird_fx then
                inst.thunderbird_fx.AnimState:PlayAnimation("charge_pst")
            end
    ...
    State{
        ...
        onenter = function(inst)
            if inst.thunderbird_fx then
                inst.thunderbird_fx.AnimState:PlayAnimation("shoot")
            end
