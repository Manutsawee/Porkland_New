...
local actionhandlers = {
    ActionHandler(ACTIONS.ABSORBWATER, "drink_pre"),
    ActionHandler(ACTIONS.SPECIAL_ACTION, "poop"),
...
local events = {
    ...
    EventHandler("attacked", function(inst)
        ...
        local is_ball = inst.sg:HasStateTag("ball")

        if health_precent > 0 and not inst.sg:HasStateTag("attack") then
            if health_precent >= 0.5 and not is_ball then
                inst.sg:GoToState("ball_pre")
            elseif health_precent < 0.5 and is_ball then
...
local states = {
    State{
        ...
        onenter = function(inst, pushanim)
            ...
            if math.random() < 0.2 and not inst.components.combat.target then
