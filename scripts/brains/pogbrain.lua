...
local MUSTONEOF_TAGS = {"player", "monster", "scarytoprey"}
...
local function GetFaceTargetFn(inst)
    local target = FindEntity(inst, START_FACE_DIST, nil, nil, CANT_TAGS, CANT_TAGS)
...
local function GetPlayerTarget(inst, distance)
    local target = FindEntity(inst, distance, nil, nil, {"playerghost", "INLIMBO"}, {"player", "monster", "scarytoprey"})
    if target then
        return target
    end
end
...
local function harassPlayer(inst)
    ...
    if target and target.components.inventory then
        p_pt = Vector3(target.Transform:GetWorldPosition())
        m_pt = Vector3(inst.Transform:GetWorldPosition())

        item = target.components.inventory:FindItem(function(item) return inst.components.eater:CanEat(item) end )
    end

    if item and distsq(p_pt, m_pt) < FOLLOWPLAYER_DIST * FOLLOWPLAYER_DIST and not (target and target.components.driver and target.components.driver:GetIsDriving())
    and inst.components.combat and not inst.components.combat.target then
        return target
    end
end
...
function PogBrain:OnStart()
    ...
    PriorityNode(
    {
        ...
        IfNode(function() return self.inst.components.combat.target and self.inst.components.combat.target:HasTag("player") and TheWorld.state.isaporkalypse end, "Chase And Attack",
